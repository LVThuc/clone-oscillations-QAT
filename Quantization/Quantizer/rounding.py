...
class EWGSDiscretizer(ParametrizedGradEstimatorBase):
    ...
    def forward(self, x):
        safe_scaling_factor = torch.clamp(self.scaling_factor, min=1e-8)
        return EWGSRounding.apply(x, safe_scaling_factor)
...
def grad_estimator(method: str, *args, **kwargs):
    ...
    if method == "STE":
        ...
        # Nếu không có args, mặc định scaling_factor = 0.01
        scaling_factor = args[0] if args else 0.01
        x = EWGSDiscretizer(scaling_factor)
        x.make_grad_params_trainable()
        return x
    else:
