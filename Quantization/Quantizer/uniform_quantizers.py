...
class AsymmetricUniformQuantizer(QuantizerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert scale_domain in ["linear", "log"], (
            "scale_domain must be 'linear' or 'log'"
        )
    ...
    def make_range_trainable(self):
        """
        Converts trainable range parameters to nn.Parameter if they are not already.
        This is the correct and recommended way.
        """
        # Kiểm tra trực tiếp xem _delta có phải là một nn.Parameter hay không
        if not isinstance(self._delta, torch.nn.Parameter):
            self._delta = torch.nn.Parameter(self._delta)

        # Làm tương tự cho _zero_float
        if not isinstance(self._zero_float, torch.nn.Parameter):
