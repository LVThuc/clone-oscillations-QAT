...
def quantize_sequential(
    model, specials=None, tie_activation_quantizers=False, **quant_params
):
    ...
    while i < len(model):
        if isinstance(model[i], QuantizedModule):
            ...
            if input_quantizer and tie_activation_quantizers:
                ...
                quant_modules.append(
                    QuantizedActivationWrapper(model[i], **quant_params)
                )
                if tie_activation_quantizers:
                    warnings.warn(
                        "Input quantizer not found, so we do not tie quantizers"
                    )
        else:
            quant_modules.append(
