...
class CurrentMinMaxEstimator(RangeEstimatorBase):
    ...
    def forward(self, x):
        ...
        if self.percentile:
            ...
            self.current_xmin = (
                x.min(-1)[0].detach() if self.per_channel else x.min().detach()
            )
            self.current_xmax = (
                x.max(-1)[0].detach() if self.per_channel else x.max().detach()
            )
...
class RunningMinMaxEstimator(RangeEstimatorBase):
    ...
    def forward(self, x):
        ...
        if self.current_xmin is None:
            ...
            self.current_xmin = (
                self.momentum * self.current_xmin + (1 - self.momentum) * x_min
            )
            self.current_xmax = (
                self.momentum * self.current_xmax + (1 - self.momentum) * x_max
            )
...
class MSE_Estimator(RangeEstimatorBase):
    ...
    def __init__(self, num_candidates=100, range_margin=0.5, *args, **kwargs):
        ...
        self.max_int_skew = (
            2**self.quantizer.n_bits
        ) // 4  # for asymmetric quantization
    ...
    def _define_search_range(self, data):
        ...
        if self.one_sided_dist or self.quantizer.symmetric:
            ...
            self.max_pos_thr = (
                max(abs(float(data.min())), float(data.max())) + self.range_margin
            )
            ...
            self.loss_array[:, 0, :, :] = (
                np.inf
            )  # exclude interval_start=interval_finish
    ...
    def _perform_1D_search(self, data):
        ...
        xmin = (
            np.zeros(self.channel_groups)
            if self.one_sided_dist
            else -self.step_size * min_cand
        ).astype(np.single)
    ...
    def _perform_2D_search(self, data):
        ...
        for cand_index in range(1, self.num_candidates + 1):
            ...
            temp_delta = float(temp_finish - temp_start) / (
