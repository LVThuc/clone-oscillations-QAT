...
def separate_quantized_model_params(quant_model):
    ...
    model_params = [
        p for p in quant_model.parameters() if not tensor_in_list(p, found_params)
    ]
    model_param_names = [
        n
    ...
    assert len(model_params + quant_params + grad_params) == len(
        ...
        len(quant_params),
        len(grad_params),
        len(list(quant_model.parameters())),
    )
...
def pass_data_for_range_estimation(
    ...
    if cross_entropy_layer is not None:
        ...
        if layer_xent:
            print(
                'Set cross entropy estimator for layer "{}"'.format(cross_entropy_layer)
            )
