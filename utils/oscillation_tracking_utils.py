...
class TrackOscillation:
    ...
    def __init__(
        ...
        self.freeze_threshold = (
            freeze_threshold  # This should be at least 2-3x the momentum value.
        )
        ...
        self.ema_x_int = None
        self.freeze = freeze
        self.ratio_above_threshold = 0.0
        self.frozen_ratio = 0.0

    def __call__(self, x_float, skip_tracking=False, *args, **kwargs):
        x_int = self.int_fwd(x_float, *args, **kwargs)

        # Apply weight freezing (nếu bật freeze và đã có mask frozen)
        ...
        with torch.no_grad():
            ...
            # Freeze some weights (chỉ chạy phần này nếu self.freeze bật)
            freeze_weights = torch.zeros_like(x_int, dtype=torch.bool)
            if self.freeze_threshold > 0:
                freeze_weights = (self.ema_oscillation > self.freeze_threshold) | self.frozen
                self.frozen[freeze_weights] = True
                if self.use_ema_x_int:
                    self.ema_x_int = (
                        self.momentum * x_int + (1 - self.momentum) * self.ema_x_int
                    )
                    self.frozen_x_int[freeze_weights] = torch.round(
                        self.ema_x_int[freeze_weights]
                    )
                else:
    ...
    def check_init(self, x_int):
        if self.prev_x_int is None:
            ...
            assert self.prev_x_int.shape == x_int.shape, (
...
def add_oscillation_trackers(
    model, max_bits=4, *args, **kwarks
):  # gắn các tracker vào model inplace
    tracker_dict = {}
    # Add oscillation trackers to all weight quantizers #thay quantizer của weight bằng TrackOscillation
    for name, module in model.named_modules():
        if isinstance(module, QuantizationHijacker):
            ...
            if q.n_bits > max_bits:
                ...
                continue
            int_fwd_wrapper = TrackOscillation(
                int_fwd=q.to_integer_forward, *args, **kwarks
            )
            q.to_integer_forward = int_fwd_wrapper
            tracker_dict[name + ".weight_quantizer"] = int_fwd_wrapper
    return tracker_dict  # mấy cái bị gắn tracker
